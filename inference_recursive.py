...
if __name__ == "__main__":
    ...
    for vid_path in video_files:
        ...
        print(f"[MEM] Prima di {os.path.basename(vid_path)}: {mem_before:.2f} MB")

        # gestione dinamica fps
        fps_try = fps
        while True:
            try:
                if args.data_type == "RGBD":
                    npz_dir = os.path.join(args.data_dir, f"{video_name}.npz")
                ...
                with torch.amp.autocast(device_type="cuda", dtype=torch.bfloat16):
                    ...
                    print(f"[MEM] Dopo {os.path.basename(vid_path)}: {mem_after:.2f} MB")
